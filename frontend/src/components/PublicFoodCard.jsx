/**
 * PublicFoodCard — works for both logged-in and logged-out users.
 * - Logged-out: clicking "Add to Cart" or "♡" shows an auth prompt modal.
 * - Logged-in: behaves identically to the original FoodCard.
 */
import { Star, Clock, Heart, Plus, Flame, Lock } from 'lucide-react'
...
export default function PublicFoodCard({ item, onFavoriteToggle }) {
  const { user }    = useAuth()
  ...
  const handleAdd = async (e) => {
    e.stopPropagation()
    if (!user) { requireAuth('cart'); return }
    setAdding(true)
    ...
    <>
      ...
      >
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-gray-100 flex-shrink-0">
          ...
          <button
            onClick={handleFav}
            aria-label={fav && user ? 'Remove from wishlist' : 'Add to wishlist'}
          ...
          >
            <Heart className={`w-4 h-4 ${fav && user ? 'fill-red-500 text-red-500' : 'text-gray-400'}`} />
          </button>
        </div>
          ...
          <div className="flex items-center justify-between mt-auto pt-3">
            ...
            {item.available ? (
              <button
                ...
                className="w-8 h-8 bg-brand-500 hover:bg-brand-600 text-white rounded-full flex items-center justify-center shadow-md transition-all disabled:opacity-60"
              >
                {adding
